// Weekly basket data refresh — end-to-end for basket 2026-07-13 / expiry 2026-07-17.
// Derived from _run_weekly_refresh_2026_07_06.mjs.
  ...
  for (const { ticker, price } of tickers) {
    try {
      const hist = await yf.chart(ticker, {
        period1: histStart, period2: now, interval: '1d',
      });
      ...
      for (const c of calls) {
        ...
        if (
          ...
          mid != null && mid > 0.01 && c.bid != null && c.bid > 0
        ) {
      ...
      for (const p of puts) {
        ...
        if (
          delta != null && delta <= -0.13 && delta >= -0.22 &&
          mid != null && mid > 0.01 && p.bid != null && p.bid > 0
        ) {
    ...
    if (done % 20 === 0)
      process.stdout.write(`\r[3] processed ${done}/${tickers.length} (err:${errors.length})`);
