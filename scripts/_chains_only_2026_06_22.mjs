// Step-3-only chain + IV fetch for basket date 2026-06-22 (expiry 2026-06-26).
...
console.log(`[3] done. summary rows: ${summaryRows.length-1} errors: ${errors.length}`);

// manifest
const macroQ = fs.readFileSync(path.join(OUT,'macro_quotes.csv'),'utf8').split('\n');
function mq(t){const row=macroQ.find(r=>r.startsWith(t+','));if(!row)return null;return parseFloat(row.split(',')[1]);}
const manifest = {
  run_ts: new Date().toISOString(),
  basket_date: BASKET_DATE,
  expiry: EXPIRY_ISO,
  macro: { SPY: mq('SPY'), VIX: mq('^VIX'), SKEW: mq('^SKEW'), MOVE: mq('^MOVE'), SPX: mq('^GSPC') },
  chains: { processed: done, errors: errors.length },
};
fs.writeFileSync(path.join(OUT,'refresh_manifest.json'), JSON.stringify(manifest,null,2));
