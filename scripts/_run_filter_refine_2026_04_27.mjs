// Copy of _run_filter_refine.mjs adapted to the current sandbox path
// and the 2026-04-27 basket folder. Writes shortlist_calls.csv,
// shortlist_puts.csv, shortlist_calls_refined.csv, shortlist_puts_refined.csv.
...
console.log('=== CALL refined (top 25) ===');
console.log('TKR   PX     IV%  HVR   K     CR    IV    ATR  BUF   OTMVol  MC($B)  Name');
for (const r of calls.slice(0, 25)) {
  console.log([
    r.ticker.padEnd(5),
    ...
    (r.market_cap / 1e9).toFixed(1).padStart(6),
    (r.name || '').slice(0, 36),
  ].join('  '));
}
...
console.log('\n=== PUT refined (top 25) ===');
console.log('TKR   PX     IV%  HVR   K     CR    IV    ATR  BUF   OTMVol  MC($B)  Name');
for (const r of puts.slice(0, 25)) {
  console.log([
    r.ticker.padEnd(5),
    r.price.toFixed(2).padStart(6),
    (r.atm_iv * 100).toFixed(0).padStart(3),
