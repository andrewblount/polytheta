...
import { createBroker, brokerRuntime } from './index.mjs';
...
import { localWorkerIdentity, directDatabaseUrl, restartWindow, createExecutionFence, chooseModeJournal } from './host-runtime.mjs';
...
try {
  ...
  await fence.assert();
  selected = true;
  const profile = brokerRuntime(settings);
  enabled = !checkOnly && profile.enabled;
  broker = createBroker(settings);
  const journalPath = path.join(runtime, profile.journalFile);
  const [hosted] = await sql`select value from app_settings where key=${profile.journalKey}`;
  const [legacy] = await sql`select value from app_settings where key='ib_execution_journal'`;
  journal = chooseModeJournal(profile.mode, {
    remote: hosted?.value, local: fs.existsSync(journalPath) ? readJournal(journalPath) : undefined,
    legacyRemote: legacy?.value, legacyLocal: readJournal(path.join(runtime, 'ib-execution.json')),
  });
  ...
  // The published MODEL basket is read from the database, so execution never
  // depends on the file system of the computer that built it. The local file
  // is only a fallback for a machine that published moments ago.
  const week = entryWeek(settings);
  const [stored] = await sql`select value from app_settings where key=${`model_basket:${week}`}`;
  const file = path.join(root, 'baskets', week, 'data', 'basket_proposal.json');
  const local = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8')) : null;
  const proposal = stored?.value?.phase === 'final' ? stored.value : local?.phase === 'final' ? local : null;
  const proposalSource = stored?.value?.phase === 'final' ? 'database' : local?.phase === 'final' ? 'local-file' : null;
  ...
  const result = await executionCycle({ broker, proposal, settings, journal, enabled, beforeWrite, authorizedEntryWeek: profile.authorizedEntryWeek,
    ...
    save: async value => {
      ...
      await persistSetting(profile.journalKey, value);
  ...
  // Paper and live fills are both recorded, labelled by broker mode, so the
  // account-versus-model report can be produced for either account.
  const positionFor = async (c, week) => {
    const rows = await sql`select p.id from positions p join baskets b on b.id=p.basket_id
      where b.slug=${`weekly-basket-${week}`} and p.ticker=${c.symbol} and p.side=${c.side} and p.strike=${c.strike} and p.expiry=${c.expiry} limit 1`;
    return rows[0]?.id ?? null;
  };
  for (const fill of Object.values(journal.fills)) {
    if (checkOnly || !profile.importLedger) break;
    ...
    const week = Object.values(journal.intents).find(i => i.ref === fill.ref)?.week ?? null;
    const positionId = week ? await positionFor(c, week) : null;
    const note = `IB execution ${fill.executionId}; modeled credit ${fill.modeledCredit ?? 'n/a'}; execution service v1`;
    await beforeWrite();
    const rows = await sql`insert into trades (id,position_id,ticker,side,action,strike,expiry,quantity,price,fees,broker,executed_at,notes)
      select ${id},${positionId},${c.symbol},${c.side},${fill.action === 'entry' ? 'sell-to-open' : 'buy-to-close'},${c.strike},${c.expiry},${fill.quantity},${fill.price},${fill.commission},${mode === 'paper' ? 'IBKR paper' : 'IBKR live'},${at},${note}
      where ${fence.predicate()}
      on conflict(id) do update set position_id=coalesce(excluded.position_id, trades.position_id),price=excluded.price,fees=excluded.fees,quantity=excluded.quantity,executed_at=excluded.executed_at,notes=excluded.notes,updated_at=now() returning id`;
    fence.checkedWrite(rows);
  }
  ...
  const status = { connected: connectedThisCycle, mode: settings?.accountMode, hostId: host.id, hostLabel: host.label, connection: settings?.connection, activated: enabled, message: selected && restartWindow(settings) ? `Expected TWS restart window; reconnecting on subsequent cycles. ${message}` : message, checkedAt: new Date().toISOString() };
  if (selected) {
    try {
      await fence.assert();
      const rows = await sql`insert into app_settings(key,value,updated_at)
