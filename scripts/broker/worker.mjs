...
try {
  ...
  const status = { ...result, account: undefined, hostId: host.id, hostLabel: host.label, connection: settings.connection, activated: enabled, checkedAt: new Date().toISOString(), fills: Object.keys(journal.fills).length };
  ...
  for (const fill of Object.values(journal.fills)) {
    ...
    const rows = await sql`insert into trades (id,ticker,side,action,strike,expiry,quantity,price,fees,broker,executed_at,notes)
      select ${id},${c.symbol},${c.side},${fill.action === 'entry' ? 'sell-to-open' : 'buy-to-close'},${c.strike},${c.expiry},${fill.quantity},${fill.price},${fill.commission},'IBKR live',${at},${note}
