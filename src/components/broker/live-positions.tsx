...
import { EntryPricingDetails } from "./entry-pricing-details";
type LossStop = { status?: string | null; baselineEquity?: number | null; thresholdAmount?: number | null; lossAmount?: number | null; lossPct?: number | null; triggeredAt?: string | null; message?: string | null };
type Position = { conid: number; ticker: string; side: string; strike: number; expiry: string; quantity: number; averageFill: number | null; mark: number | null; unrealizedPnl: number | null; realizedPnl: number; fees: number; status: string; canExit: boolean; workingEntry: boolean; entryPricing?: EntryPricingData | null; lossStop?: LossStop | null };
type State = { stale: boolean; snapshot: { accountKey: string; mode?: string; observedAt: string; activated: boolean; positions: Position[]; unrealizedPnl: number | null; realizedPnl: number; fees: number; complete: boolean } | null; requests: { requestId: string; requestedAt: string; status: string; message: string }[] };
const money = (value: number | null | undefined) => value == null ? "Unavailable" : value.toLocaleString("en-US", { style: "currency", currency: "USD" });
function TickerLossStop({ stop }: { stop: LossStop }) {
  return <div className="space-y-2 rounded-xl border p-3 text-sm">
    <p className="font-semibold">Ticker loss stop</p>
    <p>{stop.message ?? stop.status ?? "Awaiting worker monitoring"}</p>
    <p>Ticker loss / limit: {money(stop.lossAmount)} / {money(stop.thresholdAmount)}</p>
    <p className="text-muted-foreground">Account equity before entry: {money(stop.baselineEquity)}</p>
    {stop.triggeredAt && <p>Triggered: {new Date(stop.triggeredAt).toLocaleString()}</p>}
  </div>;
}
export function LivePositions() {
  ...
  const active = snapshot?.positions.filter(p => p.quantity > 0 || p.workingEntry || p.status === "Reconciliation required" || p.lossStop?.triggeredAt) ?? [];
  ...
  return <div className="space-y-6">
    ...
    <p className="text-sm text-muted-foreground">“NOW” requests immediate processing by the trading Mac. It does not guarantee an immediate fill. Outside exchange hours, exits wait for the next session. The worker monitors news and configured ticker-loss exits; these buttons are your manual override.</p>
  </div>;
}
